...
$music_count = -1;
$msr = null;
$data = db::get_string_all("SELECT `MUSIC`.`ID`,`MUSIC`.`EXT`,`MUSIC`.`NAME`,`MUSIC`.`ARTIST`,`MUSIC`.`DURATION` FROM `MUSIC` LEFT JOIN `MUSIC_DIR` ON (`MUSIC_DIR`.`ID` = `MUSIC`.`ID_DIR` OR `MUSIC_DIR`.`ID_DIR` = `MUSIC`.`ID_DIR`) WHERE `PRIVATE` = '0' GROUP BY `MUSIC`.`ID` ORDER BY `MUSIC`.`TIME` DESC LIMIT ".$limit.", ".PAGE_SETTINGS);
