<?php 
$dir = db::get_string("SELECT `ID` FROM `FILES_DIR` WHERE `ID` = ? AND `USER_ID` = ? AND `PRIVATE` != ? LIMIT 1", [intval(get('dir')), user('ID'), 3]);  
...
if (post('ok_files_album')){
  ...
  if (db::get_column("SELECT COUNT(*) FROM `FILES_DIR` WHERE `USER_ID` = ? LIMIT 1", [user('ID')]) >= config('FILES_DIR_LIMIT')){
  ...
  if (db::get_column("SELECT COUNT(*) FROM `FILES_DIR` WHERE `USER_ID` = ? AND `NAME` = ? AND `ID_DIR` = ? LIMIT 1", [user('ID'), ALBUM_NAME, intval($dir['ID'])]) == 1){
  ...
  db::get_set("INSERT INTO `FILES_DIR` (`NAME`, `PRIVATE`, `USER_ID`, `ID_DIR`, `PASSWORD`, `PASSWORD_SHOW`) VALUES (?, ?, ?, ?, ?, ?)", [ALBUM_NAME, $private, user('ID'), intval($dir['ID']), $password, ALBUM_PASSWORD]);
...
<?=html::button('button ajax-button', 'ok_files_album', 'plus', 'Добавить')?>  
