...
if (post('ok_edit_files_album')){
  ...
  if ($dir['NAME'] != ALBUM_NAME && db::get_column("SELECT COUNT(*) FROM `FILES_DIR` WHERE `USER_ID` = ? AND `NAME` = ? AND `ID_DIR` = ? LIMIT 1", [$dir['USER_ID'], ALBUM_NAME, $dir['ID']]) == 1){
    
    error('Альбом с таким названием уже существует в этой директории');
    redirect('/m/files/edit_dir/?dir='.$dir['ID'].'&'.TOKEN_URL);
  ...
  db::get_set("UPDATE `FILES_DIR` SET `PRIVATE` = ?, `NAME` = ?, `PASSWORD` = ?, `PASSWORD_SHOW` = ? WHERE `ID` = ? LIMIT 1", [$private, ALBUM_NAME, $password, ALBUM_PASSWORD, $dir['ID']]);
  
  if (access('files', null) == true){
    
    logs('Файлы - редактирование альбома [url=/m/files/users/?id='.$dir['USER_ID'].'&dir='.$dir['ID'].']'.$dir['NAME'].'[/url]', user('ID'));
    
  }
  
  success('Изменения успешно приняты');
  redirect('/m/files/users/?id='.$dir['USER_ID'].'&dir='.$dir['ID']);
  
}

?>    
<div class='list'>
<form method='post' class='ajax-form' action='/m/files/edit_dir/?dir=<?=$dir['ID']?>&<?=TOKEN_URL?>'>
...
<?=html::button('button ajax-button', 'ok_edit_files_album', 'save', 'Сохранить')?>  
<a class='button-o' href='/m/files/users/?id=<?=$dir['USER_ID']?>&dir=<?=$dir['ID']?>'><?=lg('Отмена')?></a>
<form>
</div>
<?
  
back('/m/files/users/?id='.$dir['USER_ID'].'&dir='.$dir['ID']);  
acms_footer();