...
livecms_header(lg('Общие друзья с %s', user::login_mini($account['ID'])));
...
<a class='menu-nav h' href='/account/friends/general/?id=<?=$account['ID']?>'>
...
define('URL_FRSCB', '/account/friends/?id='.$account['ID']);
require_once (ROOT.'/modules/users/plugins/friends_and_subscribers.php');  

$spage = spage($general, PAGE_SETTINGS);
$page = page($spage);
...
$data = db::get_string_all("SELECT a.`USER_ID` FROM `FRIENDS` AS a INNER JOIN `FRIENDS` as b ON (b.`USER_ID` = a.`USER_ID` AND b.`MY_ID` = ? AND b.`USER_ID` != ?) WHERE a.`MY_ID`= ? AND b.`USER_ID` != ? AND a.`ACT` = '0' AND b.`ACT` = '0' GROUP BY b.`ID`,a.`ID` LIMIT ".$limit.", ".PAGE_SETTINGS, [$account['ID'], $account['ID'], user('ID'), user('ID')]);
...
get_page('/account/friends/?id='.$account['ID'].'&', $spage, $page, 'list');
