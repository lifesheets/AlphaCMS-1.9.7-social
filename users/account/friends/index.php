...
livecms_header(lg('Друзья %s', user::login_mini($account['ID'])));
...
<a class='menu-nav h' href='/account/friends/?id=<?=$account['ID']?>'>
...
$data = db::get_string_all("SELECT * FROM `FRIENDS` WHERE `MY_ID` = ? AND `ACT` = '0' ORDER BY `ID` DESC LIMIT ".$limit.", ".PAGE_SETTINGS, [$account['ID']]);
