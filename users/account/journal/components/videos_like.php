...
<a href='/id<?=$account['ID']?>'><b><?=$account['LOGIN']?></b></a> <?=lg('оценил(-а) ваше видео')?> -   
  
<?php

$video = db::get_string("SELECT `ID`,`NAME` FROM `VIDEOS` WHERE `ID` = ? LIMIT 1", [$list['OBJECT_ID_LIST']]);  
if (!isset($video['ID'])){
  ...
  <?=tabs(crop_text($video['NAME'], 0, 40))?>  
  </a>
  <?
