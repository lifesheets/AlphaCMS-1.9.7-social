...
<a href='/id<?=$account['ID']?>'><b><?=$account['LOGIN']?></b></a> <?=lg('прокомментировал(-а) вашу тему')?> -   
  
<?php

$them = db::get_string("SELECT `ID`,`COMMUNITY_ID`,`NAME` FROM `COMMUNITIES_FORUM_THEM` WHERE `ID` = ? LIMIT 1", [$list['OBJECT_ID_LIST']]);  
if (!isset($them['ID'])){
