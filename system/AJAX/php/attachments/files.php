...
if (ajax() == true){
  ...
  if (config('PRIVATE_FILES')){
    ...
    ?>
    <div class="modal_bottom_icons_optimize modal_bottom_title_active"><span class="modal_bottom_icons_circle"><?=icons('file', 20)?></span><span class="modal_bottom_title_text"><?=lg('Файлы')?></span></div>
    <?
  ...
  if (config('PRIVATE_FILES')){
    ...
    file::upload('/files/receiver/attachments_files.php?type='.$type.'&id='.$id.'&url='.$url.'&show='.intval(get('show')));
    
    ?><div id="files_list"><?                                  
      
    $data = db::get_string_all("SELECT `ID`,`NAME`,`EXT` FROM `FILES` WHERE `USER_ID` = ? ORDER BY `TIME` DESC LIMIT 24", [user('ID')]);
    while ($list = $data->fetch()){
      
      ?><label onclick="checkbox(<?=$list['ID']?>)" class="checkbox-optimize2"><div class="list-menu"><span class="checkbox<?=$list['ID']?> checkbox-op-file check-close"><?=icons('check', 20)?></span><input type='checkbox' class='attachments-photos-checkbox' name='files' value='<?=$list['ID']?>' id='chset<?=$list['ID']?>'><?=file::ext($list['EXT'])?> <span class='attachments-files-name'><?=crop_text(tabs($list['NAME']), 0, 28)?></span></div></label><?
      
    }
    
    ?></div><?
    
    if (db::get_column("SELECT COUNT(*) FROM `FILES` WHERE `USER_ID` = ? LIMIT 1", [user('ID')]) > 24) {
      
      ?><br /><center><span onclick="show_more('/system/AJAX/php/attachments/files_list.php', '#show_more', '#files_list', 24, 'append')" class="button" id="show_more" count_show="24" count_add="24" name_show="<?=lg('Показать еще')?>" name_hide="<?=lg('Конец')?>"><?=lg('Показать еще')?></span></center><br /><? 
    ...
    <center><span onclick='modal_bottom_close()' class='modal-bottom-button'><?=lg('Отменить')?></span> <span class="bclose modal-bottom-button-at" id="get_check" type="files" idt="<?=$id?>" ptype="<?=$type?>" action="<?=base64_decode($url)?>"><?=lg('Прикрепить')?></span></center>
