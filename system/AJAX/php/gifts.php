...
if (ajax() == true){
  ...
  $dir = db::get_string("SELECT `ID` FROM `GIFTS_DIR` WHERE `ID` = ? LIMIT 1", [intval(get('id_dir'))]);
  $url = tabs(get('url'));
  ...
  if (isset($dir['ID'])){
    ...
    <?=lg('Загрузить новые подарки')?>
    </div>
    
    <div class="modal-bottom-container">      
    <?=file::upload('/files/receiver/gifts.php?id_dir='.$dir['ID'].'&url='.$url)?>
    </div>
